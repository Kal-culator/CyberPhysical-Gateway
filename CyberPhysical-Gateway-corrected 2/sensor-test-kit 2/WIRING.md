...
This table is the agreed breadboard mapping used by the corrected project. Confirm the physical wires still match it before applying power. Do not confuse physical header numbers with BCM GPIO numbers. Power off before moving wires.
...
```text
12V supply + ---- relay channel 2 COM
relay channel 2 NO ---- lock +
lock - --------------- 12V supply -
```

COM/NO are the isolated switching contacts, not VCC/IN/GND. Channel 1 switches an active 5V buzzer; channel 2 switches the lock. Using NO means each load is unpowered when its relay is off. The linked lock retracts when energised and extends when unpowered. It is listed as 12V/1.1A with intermittent energisation. [Same-ASIN uxcell specification](https://www.amazon.com/dp/B07TMWY94C)
