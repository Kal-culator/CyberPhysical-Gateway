...
RELAY_PIN = 27       # pin 13, NPN driver to relay channel 1
BUZZER_PIN = 23      # pin 16, NPN driver to 5V B-10N buzzer
...
BUZZER_KIND = os.environ.get("BUZZER_KIND", "passive").lower()
...
def sound_buzzer(pwm, seconds=0.8):
    try:
        if BUZZER_KIND == "active":
...
def main():
    if BUZZER_KIND not in ("active", "passive"):
    ...
    print(f"Buzzer mode: {BUZZER_KIND}")
    try:
        while True:
